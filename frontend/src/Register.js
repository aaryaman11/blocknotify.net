...
function SignForm() {
    ...
    const [signedPhone, setSignedPhone] = React.useState('');

    React.useEffect(() => {
        if (signedPhone && signedPhone !== "") {
            axios
                .post("http://localhost:8000/api/register", {
                    "phone": targetPhone, "signature": signedPhone
                })
                .then((res) => {
                    ...
                    // TODO: figure out how to reload the app now...
                    // ReactDOM.render(<App/>);
                })
    ...
    const sign = () => {
        try {
            ...
            signer
                .signMessage(getMessageToSign(targetPhone))
                .then((signedMessage) => {
                    setSignedPhone(signedMessage);
                })
